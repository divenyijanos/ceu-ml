...
```{r create-h2o-frames}
my_seed <- 20230322
# I keep only 10% of the data to quicken up learning (only for educational purposes)
fmnist_data_small <- slice_sample(fmnist_data, prop = 0.1)

data_split <- h2o.splitFrame(as.h2o(fmnist_data_small), ratios = 0.8, seed = my_seed)
...
For some reason, accuracy cannot be extracted right away from an h2o multinomial prediction model only a similar metric: mean per-class error. You can see the accuracy though in the bottom right corner of the Confusion Matrix. Comparing it to the mean per-class error makes it clear that these two metrics are practically the same for our problem.

```{r logit-performance}
# plot the confusion matrix and calculate the mean per class error for the logit
h2o.confusionMatrix(fmnist_logit)
...
# 199,010 params = 783 (input features) * 200 (first layer nodes) + 200 (first layer biases) +
...
plot(fmnist_dl_default, metric = "logloss")
```
...
plot(fmnist_dl_adjusted, metric = "logloss")
```
...
Keras also has an R interface that we are going to use. Consult [this page](https://tensorflow.rstudio.com/install/) to install this to your local machine.
...
```{r adjust-data}
# the transformation takes a while... you can load the same data that is built-in into the keras lib by calling dataset_fashion_mnist() - we do it to ensure that the test sets are comparable
...
```{r tranform-if-started-with-keras-inbuilt-data}
# n_rows <- nrow(fmnist_keras$train$x)
# small_data_indices <- sample(seq(n_rows), floor(n_rows * 0.1))
# train_indices <- sample(small_data_indices, floor(n_rows * 0.8 * 0.1))
# data_train_x <- array_reshape(fmnist_keras$train$x[train_indices,,]/255, c(length(train_indices), 28*28, 1))
# data_valid_x <- array_reshape(fmnist_keras$train$x[setdiff(small_data_indices, train_indices),,]/255, c(n_rows - length(train_indices), 28*28, 1))
#
# data_train_y <- to_categorical(fmnist_keras$train$y[train_indices], 10)
# data_valid_y <- to_categorical(fmnist_keras$train$y[-train_indices], 10)
```
...
# table(fmnist_keras$train$y[-train_indices], predicted_labels) # if started with keras built-in data
```
