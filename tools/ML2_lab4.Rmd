...
obs_of_interest <- as_tibble(airbnb_holdout)[4575, features] 
obs_of_interest
```
...
```{r pdp-cancellation}
plot(model_profile(explainer_rf, variables = "f_cancellation_policy"))
plot(model_profile(explainer_rf, variables = "n_review_scores_rating", groups = "f_cancellation_policy")) +
    xlim(80, 100)
```


```{r create-binary-treatment}
airbnb_train$f_strict_cancellation <- as.factor(airbnb_train$f_cancellation_policy == "strict")
airbnb_holdout$f_strict_cancellation <- as.factor(airbnb_holdout$f_cancellation_policy == "strict")
treatment_variable <- "f_strict_cancellation"
features_base <- setdiff(features, c("f_cancellation_policy", treatment_variable))
```
...
n_sim <- 10
rct_results <- runSimulations(n_sim, "RCT", VERSION_PROBS)
ect10_results <- runSimulations(n_sim, "ECT", VERSION_PROBS, policy_params = list(explore_until = 10))
ect50_results <- runSimulations(n_sim, "ECT", VERSION_PROBS, policy_params = list(explore_until = 50))
...
ggplot(ect10_results, aes(x = i, y = showed_version, group = run)) +
    geom_line(alpha = 0.1)
ggplot(ect10_results, aes(x = i, y = showed_version, group = run)) +
    geom_line(alpha = 0.1)
```
